...
//	HIT	NUMBER(4,0)
	...
	{ 
		...
		String query="SELECT * FROM JSPBOARD ORDER BY BOARD_ID DESC"; //최신 글은 밑으로 내려가야하므로 내림차순으로 정렬한다
		...
		try
		{
			...
			rs = pstmt.executeQuery(); //select문이므로 executeQuery로 사용한다.
...
//				HIT	NUMBER(4,0)
	...
	public int boardInsert(BoardDTO dto) 
	{
		...
		String query="INSERT INTO JSPBOARD (BOARD_ID, WRITER, TITLE, CONTENT) "+
		...
		try
		{
			...
			//insert문은 executeUpdate()문 사용, 반환타입: int
	...
	{	
		...
		String query="SELECT * FROM JSPBOARD WHERE BOARD_ID = ?";
...
//			HIT	NUMBER(4,0)
	...
	{
		String query="UPDATE JSPBOARD SET HIT = HIT + 1 WHERE BOARD_ID = ?";
	...
	public int boardUpdate(BoardDTO dto) 
	{
		...
		String query ="UPDATE JSPBOARD SET TITLE = ?, CONTENT = ? WHERE BOARD_ID = ?";
